...
stock Elevator_Initialize()
{
	...
	Label_Elevator = Create3DTextLabel("{CCCCCC}Presiona '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para usar el elevador.", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0, 0, 1);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Presiona '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' para llamar el elevador.", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialogEx(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "LS BeachSide Elevador...", string, "Aceptar", "Cancekar");
