...
new ElevatorQueue[4],  	// Floors in queue.
	FloorRequestedBy[4];   // FloorRequestedBy[floor_id] = playerid; - Points out who requested which floor.
...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Presionar '{00C0FF}Y{CCCCCC}' para usar el ascensor.", 0xCCCCCCAA, 1784.9822, -1302.0426, z - 0.9, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	/*if()*/
/*	printf("FS");
	if(GetPlayerDialogAC(playerid) != dialogid) return Kick(playerid);
	SetPVarInt(playerid, "DialogSegureAC", -1);*/
...
stock Elevator_Initialize()
{
	...
	Label_Elevator          = Create3DTextLabel("{CCCCCC}Presionar '{00C0FF}Y{CCCCCC}' pasar usar el ascensor.", 0xCCCCCCAA, 1784.9822, -1302.0426, 13.6491, 4.0, 0, 1);
	...
	for(new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		Obj_FloorDoors[i][1] 	= CreateObject(18756, X_DOOR_CLOSED, -1303.171142, GetDoorsZCoordForFloor(i), 0.000000, 0.000000, 270.000000);
  		...
  		format(string, sizeof(string), "{BDBDBD}[%s]\n{CCCCCC}Presionar '{00C0FF}Y{FFFFFF}' para llamar el elevador.", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialogEx(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "Elevador", string, "Aceptar", "Cancelar");
